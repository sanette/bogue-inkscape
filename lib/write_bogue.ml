...
let get_action c = Option.value ~default:no_action c.I.action
...
let scolor (r, g, b, a) = sprintf "(%u,%u,%u,%u)" r g b a

let sconnection c =
  let cid = new_conn_id () in
  let action = get_action c in
  sprintf "let c%u = W.connect %s %s %s %s in" cid c.I.src c.I.dst action
    no_trigger
...
let write_button out style text =
  ...
  F.fprintf out {|@[<hov 2>W.button%s%s "%s"@]|} border_radius border_color
    text (* TODO *)
...
let write_label out label =
  ...
  F.fprintf out {|@[<hov 2>W.label ~size:%u%s ~fg:%s@ "%s"@]|} label.C.size font
    (scolor label.C.color) label.C.text
...
let widget_pos (w : C.widget) =
  (* TODO à changer ? dans C aussi *)
  match w.C.content with
  | Box r -> (r.C.x, r.C.y)
...
let line_width x = if x <= 0 then 0 else if x <= 1 then 1 else x
...
let write_bg out style =
  ...
  | Some (r, g, b, a), None, _ | Some (r, g, b, a), _, None -> (
      ...
      | Some radius ->
          if a <> 255 && radius <> 0 then
            print_endline
              "Warning: currently, a border with positive [radius] is not \
               compatible with a transparent background.";
          Some
            (fun () ->
              F.fprintf out
                "@[<hov 2>let border =@ Style.(mk_border ~radius:%i (mk_line \
                 ~width:0 ())) in@]@;\
                 <1 0>@[<hov 2>let background =@ Style.color_bg (%u,%u,%u,%u) \
                 in@]@;\
                 <1 0>@[<hov 2>L.style_bg (Style.create ~background ~border \
                 ())@]"
                radius r g b a))
  | None, Some (r, g, b, a), Some stroke_width ->
      ...
      Some
        (fun () ->
          F.fprintf out
            "@[<hov 2>let color = %s in@]@;\
             <1 0>@[<hov 2>L.style_bg Style.(of_border (%s))@]"
            (scolor (r, g, b, a))
            border)
  | Some c, Some lc, Some stroke_width ->
      ...
      Some
        (fun () ->
          F.fprintf out
            "@[<hov 2>let color = %s in@]@;\
             <1 0>@[<hov 2>let border =@ Style.(%s) in@]@;\
             <1 0>@[<hov 2>let background =@ Style.color_bg %s in@]@;\
             <1 0>@[<hov 2>L.style_bg (Style.create ~background ~border ())@]"
            (scolor lc) border (scolor c))

let pr_x x = if x >= 0 then sprintf " ~x:%u" x else sprintf " ~x:(%i)" x
let pr_y y = if y >= 0 then sprintf " ~y:%u" y else sprintf " ~y:(%i)" y

let write_resident out name r style (wg : C.widget) =
  let name = soption "name" quote name in
  let opt =
    match write_bg out style with
    | None -> ""
    | Some f ->
        F.fprintf out "@[<hov 2>let background =@ ";
        f ();
        F.fprintf out " in@]@;<1 0>";
        " ~background"
  in
  ...
  F.fprintf out "@[<hov 2>L.resident%s%s%s%s%s%s@ %s@]@;" name opt (pr_x r.C.x)
    (pr_y r.C.y) w h wg.C.id
...
let write_rooms out name r list =
  ...
  let w = if r.C.w = 0 then "" else sprintf " ~w:%u" r.C.w in
  let h = if r.C.h = 0 then "" else sprintf " ~h:%u" r.C.h in
  F.fprintf out "@[<hov 2>L.superpose%s%s%s [%s]@]@;" name w h
    (String.concat "; " ids)

let write_layout out (room : C.layout) =
  F.fprintf out "@[<hov 2>let %s =@ " room.C.id;
  let () =
    match room.C.content with
    | Resident w -> write_resident out room.C.name room.C.rect room.style w
    | Rooms list -> write_rooms out room.C.name room.C.rect list
  in
  F.fprintf out "in@]@;<1 0>"

let write_action out name =
  F.fprintf out
    {|@[<hov 2>let %s _src _dst _ev =@;<1 2>print_endline "%s" in@]@ |} name
    name
...
let write ?file layout connections =
  ...
  let add s =
    ...
    pp_close_box out ();
    pp_print_break out 1 0
  in
  F.pp_open_vbox out 0;
  Option.iter
    (fun file ->
      F.fprintf out
        "@[<v 3>(* This file is generated by Bogue_inkscape@ from the Inkscape \
         file [%s]. *)@]@;"
        file;
      br ())
    file;
  ...
  let cs =
    Array.init (new_conn_id () - 1) (fun i -> "c" ^ string_of_int (i + 1))
