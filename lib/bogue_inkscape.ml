...
let to_string ?file (l, c) = Write_bogue.write ?file l c
let file_to_string file = to_string ~file (import file)

let convert ~output file =
  let code = file_to_string file in
  Out_channel.with_open_text output (fun outch -> output_string outch code)
