...
type transform = Translate of (float * float) | Scale of float
...
let no_style =
  { fill = None; stroke_color = None; stroke_width = None; radius = None }
...
let find_attr_opt a name =
  List.find_opt (fun ((_, n), _) -> n = name) a |> Option.map snd
...
let find_version a =
  List.find_opt
    (fun ((url, n), _) -> n = "version" && Filename.basename url = "inkscape")
    a
  |> Option.map snd
  |> Option.map (fun v -> Scanf.sscanf v "%u.%u.%u" (fun a b c -> (a, b, c)))

let find_float a name = float_of_string (find_attr a name)
let find_float_opt a name = find_attr_opt a name |> Option.map float_of_string
...
let parse_dim s =
  ...
  | None ->
      let f, unit =
        ...
        with Scanf.Scan_failure _ -> failwith "Malformed length"
      in
      let unit =
        match unit with
        | "cm" -> Cm
...
let parse_viewbox v =
  try Scanf.sscanf v "%f %f %f %f" (fun a b c d -> (a, b, c, d))
  with _ -> (
    try Scanf.sscanf v "%f , %f , %f , %f" (fun a b c d -> (a, b, c, d))
...
let parse_transform s =
  ...
  let rec loop res =
    try
      let tr =
        Scanf.bscanf inch " translate ( %f , %f ) " (fun x y ->
            Translate (x, y))
      in
      loop (tr :: res)
    with _ -> (
      try
        let sc = Scanf.bscanf inch " scale ( %f ) " (fun x -> Scale x) in
        loop (sc :: res)
      with _ -> List.rev res)
  in
  loop []
...
let global_translate tlist =
  List.fold_left
    (fun (dx, dy) -> function
      ...
      | _ ->
          print_endline "Only translate transforms are supported.";
          (dx, dy))
    (0., 0.) tlist

let extract_byte s i = int_of_string ("0x" ^ String.sub s i 2)
let extract_half s i = int_of_string ("0x" ^ String.sub s i 1)

let parse_rbg c =
  if c = "" then (
    print_endline "Empty color string";
    None)
  else if c.[0] = '#' then
    if String.length c = 7 then
      Some (extract_byte c 1, extract_byte c 3, extract_byte c 5)
    else if String.length c = 4 then
      Some (extract_half c 1, extract_half c 2, extract_half c 3)
    else (
...
let parse_color opacity c =
  ...
  parse_rbg c |> Option.map (fun (r, g, b) -> (r, g, b, opacity))

let parse_opacity = function
  | None -> None
  | Some x ->
      255. *. float_of_string x |> Float.round |> int_of_float |> Option.some
...
let style_to_list s =
  ...
  |> List.map (fun param ->
         ...
         | [ key; value ] -> (key, value)
         | _ ->
             print_endline (sprintf "Wrong parameter string [%s]" param);
             (param, ""))

let parse_style ~radius s =
  (* First convert the style string to an assoc List *)
  let map = style_to_list s in
  let opacity = List.assoc_opt "opacity" map |> parse_opacity in
  let fill =
    match List.assoc_opt "fill" map with
    | None -> None
    | Some "none" -> None
    | Some c -> parse_color opacity c
  in
  let stroke_opacity = List.assoc_opt "stroke-opacity" map |> parse_opacity in
  let stroke_width =
    List.assoc_opt "stroke-width" map |> Option.map float_of_string
  in
  let stroke_color =
    List.assoc_opt "stroke" map |> option_bind (parse_color stroke_opacity)
  in
  { fill; stroke_color; stroke_width; radius }

let parse_svg a =
  if find_attr_opt a "inkscape" = None then
    failwith "This does not seem to be an Inkscape file";
  ...
  let x, y, w, h = find_attr_opt a "viewBox" |> Option.get |> parse_viewbox in
  ...
  let viewport =
    { id; label; title = None; desc = None; x; y; w; h; style = no_style }
  in
  (version, { viewport; width; height; xscale; yscale })

let option_to_list = function Some l -> l | None -> []
...
let parse_group a =
  ...
  let smode =
    match mode with None -> "" | Some mode -> sprintf "mode=%s, " mode
  in
  print_endline (sprintf "Group %sid=%s" smode id);
  let tlist =
    find_attr_opt a "transform" |> Option.map parse_transform |> option_to_list
  in
  ( {
      id;
      label;
...
let parse_path a : connection option =
  ...
  | Some src ->
      let dst =
        match find_attr_opt a "connection-end" with
        | Some dst -> dst
        | None -> src
      in
...
let parse_rect a : rect =
  ...
  let style =
    find_attr_opt a "style"
    |> Option.map (parse_style ~radius:ry)
    |> Option.value ~default:no_style
  in
  let label = find_attr_opt a "label" in
  let slabel = Option.value ~default:"" label in
  print_endline
    (sprintf "Rectangle id=%s %s (%f, %f, %f, %f)" id slabel x y w h);
  { id; x; y; w; h; label; title = None; desc = None; style }

let parse_title _a : var = ("title", "")
...
let parse_element a = function
  | "svg" ->
      let version, canvas = parse_svg a in
      Svg (canvas, [], version)
  | "g" -> Obj (Group (parse_group a))
  | "rect" -> Obj (Rect (parse_rect a))
  | "path" -> (
      match parse_path a with None -> Ignore | Some p -> Connection p)
...
let find_var key content =
  List.filter_map (function Var v -> Some v | _ -> None) content
  |> List.assoc_opt key
...
let find_tspans content =
  List.filter_map
    (function
      | Tspan t -> Some t
      | _ ->
          print_endline "Warning: non-tspan object in text.";
          None)
    content
...
let transform_fn : transform -> rect -> rect = function
  | Translate (dx, dy) ->
      fun r ->
        ...
        { r with x; y }
  | _ ->
      print_endline "Only translate transforms are implemented.";
      fun r -> r
...
let finish_element ?(simplify = true) data content el =
  ...
  | Obj (Group (r, olist, tlist)) ->
      ...
      let olist =
        List.filter_map (function Obj o -> Some o | _ -> None) content
      in
      let o =
        List.fold_left
          (fun o tr ->
  ...
  | Svg (canvas, olist, version) ->
      let () =
        ...
        | Some (a, b, c) ->
            if not (List.mem (a, b, c) tested_versions) then
              print_endline
                (sprintf
                   "Warning: Inkscape version %u.%u.%u was not tested with \
                    this version of Bogue-inkscape."
                   a b c)
      in
      assert (olist = []);
...
let rec pull_list name data content cnxs i d : inkscape list * string =
  ...
  | `El_end ->
      print_endline
        (sprintf "end list for [%s] (%i elements), d=%i" name
           (List.length content) d);
      (content, data)
  | `Data v ->
      print_endline (sprintf "Data = %s" v);
      if data <> "" && v <> data then
        print_endline
...
let parse file =
  ...
  | [ Svg s ] -> (s, cnxs)
  | Svg s :: _rest ->
      print_endline "Selecting only the first SVG object";
      (s, cnxs)
